...
pub async fn run() -> anyhow::Result<()> {
    ...
    match args.command.unwrap_or(Command::Serve {
        ...
        Command::Install {
            ...
            no_settings,
            bin,
        } => install(client, force, keep_raw_mcp, no_settings, bin),
...
fn install(
    ...
    match client {
        ...
        Client::OmpPlugin => install_omp_plugin(force, keep_raw_mcp, no_settings, bin),
...
const OMP_PLUGIN_VERSION: &str = "1";
const OMP_PLUGIN_MARKER: &str = "// Generated by agskmem install omp-plugin. Do not edit by hand.";
const OMP_PLUGIN_TEMPLATE: &str = include_str!("../templates/omp-agskmem-extension.ts");

fn install_omp_plugin(
    force: bool,
    keep_raw_mcp: bool,
    no_settings: bool,
    bin: Option<PathBuf>,
    ...
    let home = dirs::home_dir().context("home directory not found")?;
    print_json(install_omp_plugin_at(
        &home,
        force,
        keep_raw_mcp,
        no_settings,
...
fn install_omp_plugin_at(
    ...
    let extension = home.join(".omp/agent/extensions/agskmem/index.ts");
    let settings = home.join(".omp/agent/settings.json");
    let mcp = home.join(".omp/agent/mcp.json");
    let bin = bin.unwrap_or(std::env::current_exe().context("resolving current executable")?);

    let mut updated = Vec::new();
    let mut backups = Vec::new();

    write_omp_extension(&extension, &bin, force, &mut backups)?;
    updated.push(extension);

    if !no_settings {
        merge_omp_settings_backend_off(&settings, force, &mut backups)?;
        updated.push(settings);
        if !keep_raw_mcp && disable_omp_raw_mcp(&mcp, force, &mut backups)? {
            updated.push(mcp);
        }
    }

    Ok(json!({"updated": updated, "backups": backups}))
}

fn write_omp_extension(
    path: &Path,
    bin: &Path,
    force: bool,
    ...
    if path.exists() {
        ...
        if !existing.starts_with(OMP_PLUGIN_MARKER) && !force {
            bail!(
                "refusing to overwrite non-generated extension {}; rerun with --force",
                path.display()
            );
        }
        backups.push(backup_existing(path)?);
    }
    let bin_json = serde_json::to_string(
        bin.to_str()
            .context("agskmem binary path must be valid UTF-8 for generated TypeScript")?,
    )?;
    let text = OMP_PLUGIN_TEMPLATE
        .replace("\"__AGSKMEM_BIN__\"", &bin_json)
        .replace("__PLUGIN_VERSION__", OMP_PLUGIN_VERSION);
    fs::write(path, text)?;
    Ok(())
}

fn merge_omp_settings_backend_off(
    path: &Path,
    force: bool,
    backups: &mut Vec<PathBuf>,
) -> anyhow::Result<()> {
    ...
    match root_map.get_mut("memory") {
        Some(memory) => {
            if !memory.is_object() {
                if !force {
                    bail!(
                        "settings memory entry in {} is not an object; rerun with --force",
                        path.display()
                    );
                }
                *memory = json!({});
            ...
            memory
                .as_object_mut()
                .context("settings memory entry must be object")?
                .insert("backend".to_string(), json!("off"));
        }
        None => {
            let mut memory = Map::new();
            memory.insert("backend".to_string(), json!("off"));
            root_map.insert("memory".to_string(), Value::Object(memory));
        }
    }
    if path.exists() {
        backups.push(backup_existing(path)?);
...
fn disable_omp_raw_mcp(
    ...
    if !servers.is_object() {
        if !force {
            bail!(
                "mcpServers entry in {} is not an object; rerun with --force",
                path.display()
            );
        }
        return Ok(false);
    ...
    if !server.is_object() {
        if !force {
            bail!(
                "mcpServers.agskmem entry in {} is not an object; rerun with --force",
                path.display()
            );
        }
        *server = json!({});
    ...
    let server_map = server
        .as_object_mut()
        .context("mcpServers.agskmem must be object")?;
    if server_map.get("enabled") == Some(&json!(false)) {
        return Ok(false);
    }
    server_map.insert("enabled".to_string(), json!(false));
    backups.push(backup_existing(path)?);
    fs::write(path, serde_json::to_vec_pretty(&root)?)?;
    Ok(true)
}
...
fn backup_existing(path: &Path) -> anyhow::Result<PathBuf> {
    let file_name = path
        .file_name()
        .and_then(|name| name.to_str())
        .context("backup path file name must be valid UTF-8")?;
...
mod tests {
    ...
    #[test]
    fn generated_extension_contains_marker_and_binary_path() {
        let tmp = TempDir::new().expect("tempdir");
        let path = tmp.path().join("index.ts");
        let mut backups = Vec::new();

        write_omp_extension(&path, Path::new("/tmp/agskmem test"), false, &mut backups)
            .expect("write extension");

        let text = fs::read_to_string(path).expect("read extension");
        assert!(text.starts_with(OMP_PLUGIN_MARKER));
        assert!(text.contains("// agskmem-plugin-version: 1"));
        assert!(text.contains("process.env.AGSKMEM_BIN || \"/tmp/agskmem test\""));
        assert!(backups.is_empty());
    }

    #[test]
    fn unmarked_extension_is_not_overwritten_without_force() {
        let tmp = TempDir::new().expect("tempdir");
        let path = tmp.path().join("index.ts");
        fs::write(&path, "export default function userExtension() {}\n").expect("write existing");
        let mut backups = Vec::new();

        let err = write_omp_extension(&path, Path::new("/tmp/agskmem"), false, &mut backups)
            .expect_err("unmarked extension must be refused");

        assert!(
            err.to_string()
                .contains("refusing to overwrite non-generated")
        );
        assert_eq!(
    ...
    #[test]
    fn marked_extension_is_overwritten_with_backup() {
        let tmp = TempDir::new().expect("tempdir");
        let path = tmp.path().join("index.ts");
        fs::write(&path, format!("{OMP_PLUGIN_MARKER}\nold\n")).expect("write existing");
        let mut backups = Vec::new();

        write_omp_extension(&path, Path::new("/tmp/new-agskmem"), false, &mut backups)
            .expect("overwrite generated extension");

        let text = fs::read_to_string(&path).expect("read extension");
        assert!(text.contains("process.env.AGSKMEM_BIN || \"/tmp/new-agskmem\""));
        assert_eq!(backups.len(), 1);
        assert_eq!(
            fs::read_to_string(&backups[0]).expect("read backup"),
            format!("{OMP_PLUGIN_MARKER}\nold\n")
        );
    }

    #[test]
    fn force_overwrites_unmarked_extension_with_backup() {
        let tmp = TempDir::new().expect("tempdir");
        let path = tmp.path().join("index.ts");
        fs::write(&path, "custom extension\n").expect("write existing");
        let mut backups = Vec::new();

        write_omp_extension(&path, Path::new("/tmp/agskmem"), true, &mut backups)
            .expect("force overwrite");

        assert!(
            fs::read_to_string(&path)
                .expect("read extension")
                .starts_with(OMP_PLUGIN_MARKER)
        );
        assert_eq!(backups.len(), 1);
        assert_eq!(
            fs::read_to_string(&backups[0]).expect("read backup"),
    ...
    #[test]
    fn settings_merge_preserves_unrelated_values_and_disables_memory_backend() {
        let tmp = TempDir::new().expect("tempdir");
        ...
        fs::write(
            ...
            serde_json::to_vec_pretty(&json!({
                "theme": "dark",
                "memory": {"backend": "mnemopi", "other": true}
            }))
        ...
        merge_omp_settings_backend_off(&path, false, &mut backups).expect("merge settings");

        let value: Value =
            serde_json::from_str(&fs::read_to_string(&path).expect("read settings")).expect("json");
        assert_eq!(value["theme"], "dark");
        assert_eq!(value["memory"]["backend"], "off");
        assert_eq!(value["memory"]["other"], true);
        assert_eq!(backups.len(), 1);
    }

    #[test]
    fn raw_mcp_disable_only_flips_enabled_false() {
        let tmp = TempDir::new().expect("tempdir");
        ...
        fs::write(
            ...
            .expect("serialize mcp"),
        )
        .expect("write mcp");
        let mut backups = Vec::new();

        assert!(disable_omp_raw_mcp(&path, false, &mut backups).expect("disable raw mcp"));

        let value: Value =
            serde_json::from_str(&fs::read_to_string(&path).expect("read mcp")).expect("json");
        assert_eq!(value["mcpServers"]["agskmem"]["enabled"], false);
        assert_eq!(value["mcpServers"]["agskmem"]["command"], "agskmem");
        assert_eq!(value["mcpServers"]["agskmem"]["args"], json!(["serve"]));
        assert_eq!(value["mcpServers"]["other"]["enabled"], true);
        assert_eq!(backups.len(), 1);
    }

    #[test]
    fn keep_raw_mcp_leaves_mcp_config_unchanged() {
        let tmp = TempDir::new().expect("tempdir");
        let mcp = tmp.path().join(".omp/agent/mcp.json");
        fs::create_dir_all(mcp.parent().expect("mcp parent")).expect("create dirs");
        let original = json!({"mcpServers": {"agskmem": {"command": "agskmem", "enabled": true}}});
        fs::write(
            &mcp,
            serde_json::to_vec_pretty(&original).expect("serialize mcp"),
        )
        .expect("write mcp");

        install_omp_plugin_at(
            tmp.path(),
            false,
            true,
            false,
            Some(PathBuf::from("/tmp/agskmem")),
        )
        .expect("install plugin");

        let value: Value =
            serde_json::from_str(&fs::read_to_string(&mcp).expect("read mcp")).expect("json");
